...
export default function TreasuryPage() {
  ...
  async function handlePayment(e: React.FormEvent) {
    ...
    try {
      ...
      const p = await api.get<Payment[]>('/payments');
      setPayments(p);
    } catch (err: unknown) {
    ...
    <div className="space-y-6">
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <SectionCard
          ...
          description="El monto se carga automáticamente según la categoría del H.·., pero puede ajustarse manualmente. También permite cargar comprobante por URL o nota."
        >
          <form onSubmit={handlePayment} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Ej: comprobante enviado por WhatsApp, transferencia Banco Nación..."
                className="w-full rounded-2xl border border-ink/10 px-4 py-3 text-sm"
              />
            </div>
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-ink/60">
                    Vigente desde{' '}
                    {rate.validFrom
                      ? rate.validFrom.slice(0, 10).split('-').reverse().join('/')
                      : '-'}
                  </div>
                </div>
